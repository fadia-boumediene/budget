...
class ConstruireDPICObserver
{
     ...
     * @return void
     */
    public function created(ConstruireDPIC $construireDPIC): void
    {
         //\Log::info('construireDPIC created: ' . get_class($construireDPIC));
         $this->logActivity('created', $construireDPIC);
    }
  ...
  /**
     ...
     * @return void
     */
    public function updated(ConstruireDPIC $construireDPIC): void
    {
        // Récupérer les changements effectués sur le modèle
        $changes = $construireDPIC->getChanges();

        // Récupérer les données originales avant la mise à jour
        $original = $construireDPIC->getOriginal();

        // Enregistrer les changements dans les logs
        \Log::info("Modifications sur le modèle construireDPIC", $changes);

        // Utilisez logActivity pour enregistrer l'action avec les données
        $this->logActivity('updated', $construireDPIC, $changes, $original);
    }
     ...
     * @return void
     */
    public function deleted(ConstruireDPIC $construireDPIC): void
    {
        $this->logActivity('deleted', $construireDPIC);
